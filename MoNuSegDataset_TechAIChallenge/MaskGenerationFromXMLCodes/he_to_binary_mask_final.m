...
function [binary_mask,color_mask]=he_to_binary_mask(base_dir, filename)
    im_file=strcat(base_dir, '/Tissue images/', filename,'.tif');
    
    xml_file=strcat(base_dir, '/Annotations/', filename,'.xml');
    ...
    imwrite(binary_mask, strcat(base_dir, '/Masks/', filename, '_binary_seg.png'))
    % figure;imshow(color_mask)
    imwrite(color_mask, strcat(base_dir, '/Masks/', filename, '_cell_seg.png'))
    clear
end
